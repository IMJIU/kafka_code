...
class MessageCompressionTest extends JUnitSuite {
  ...
  def testSimpleCompressDecompress(compressionCodec: CompressionCodec) {
    ...
    val messageSet = new ByteBufferMessageSet(compressionCodec,  messages:_*)
    println(compressionCodec.name+"-"+messageSet.buffer.limit());
  ...
  def testComplexCompressDecompress() {
    ...
    val message = new ByteBufferMessageSet(DefaultCompressionCodec,  messages.slice(0, 2):_*)
    val complexMessages = List(message.shallowIterator.next.message):::messages.slice(2,3)
    val complexMessage = new ByteBufferMessageSet(DefaultCompressionCodec, complexMessages:_*)
